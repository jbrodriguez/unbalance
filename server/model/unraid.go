...
func (self *Unraid) Refresh() *Unraid {
	...
	// lib.Shell("/root/mdcmd status|strings", self.readUnraidConfig, disks)

	// di := &DiskInfoDTO{Free: make(map[string]uint64), Size: make(map[string]uint64)}
	// lib.Shell("df --block-size=1 /mnt/disk", self.getDiskInfo, di)
...
func (self *Unraid) readUnraidConfig(line string, arg interface{}) {
	...
	if strings.HasPrefix(line, "diskNumber") {
		...
		if disks[diskId] == nil {
			disks[diskId] = &Disk{Id: diskId, Path: "/mnt/disk" + dn[2]}
		}
	}
	...
	if strings.HasPrefix(line, "diskName") {
		...
		mlog.Info("diskName diskId %d", diskId)
		if len(dm) > 2 {
			disks[diskId].Name = dm[2]
		} else if diskId == 0 {
			disks[diskId].Name = "Parity"
		}
	}
